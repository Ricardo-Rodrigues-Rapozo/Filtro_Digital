...
def estima_f_zc(s, Ts, Nppc, plot_level=0):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(y=f_zc, name="Reference", mode='lines'))
    fig.add_trace(go.Scatter(y=f_zc2, name="Zero Crossing", mode='lines'))
    ...
    zc_m_delay = int(np.fix((Nw + N) / 2)) + Nppc//2
    zc_delay = int(np.fix(N / 2))
