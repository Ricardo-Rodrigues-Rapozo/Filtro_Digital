...
out = np.loadtxt(OUTPUT_DIR / "output_1.txt")
...
X_real = np.loadtxt(REFERENCE_DIR / "X_rampa_real.txt")
X_imag = np.loadtxt(REFERENCE_DIR / "X_rampa_imag.txt")
freq_rampa = np.loadtxt(REFERENCE_DIR / "f_rampa.txt")
...
freq_rampa = freq_rampa[::Nppc]
freq_rampa = freq_rampa[: len(PFT[1])]
freq = freq_rampa
...
ErroAFT = 100 * np.abs(AFT - magref) / AFT
ErroPFT = np.abs(PFTc - angref) * 180 / np.pi
TVEFT = auxiliares.TVE(Xc, X_ref)
...
fig.update_layout(
    title_text="Errors for Harmonic",
